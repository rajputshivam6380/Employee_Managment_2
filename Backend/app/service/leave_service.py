...
def apply_leave(db: Session, employee_id: int, leave_data):
    ...
    if leave_data.start_date > leave_data.end_date:
        raise HTTPException(status_code=400, detail="Date cant greter than end date")
    ...
    if existing_leave:
        raise HTTPException(
            status_code=400, detail="Leave already applied for selected dates"
        )
...
def approve_leave(db: Session, leave_id: int, approved_by: int):
    ...
    if not leave:
        raise HTTPException(status_code=400, detail="Leave not found")
...
def reject_leave(db: Session, leave_id: int, approved_by: int):
    ...
    if leave.status == LeaveStatus.rejected:
        raise HTTPException(status_code=400, detail="Leave already rejected")
