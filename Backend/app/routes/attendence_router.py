...
def employee_attendance(
    ...
    return get_employee_attendence(
        db=db,
...
def attendance_by_date(
    ...
    if current_user["role"] == RoleEnum.EMPLOYEE:
        raise HTTPException(
            status_code=403,
            detail="Employees cannot view company attendance"
        )

    return get_attendence_by_date(
        db=db,
...
def today_attendence_status(
    ...
    return get_today_attendance_status(db,
                                       employee_id=get_current_user["user_id"] )
