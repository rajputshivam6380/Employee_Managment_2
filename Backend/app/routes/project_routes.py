...
async def assign_project(
    ...
    current_user=Depends(require_roles([RoleEnum.ORGANIZATION_ADMIN])),
):
    ...
    # ================= CHECK EMPLOYEE =================
    employees = db.query(User).filter(User.id.in_(assigned_to)).all()
    ...
    for emp in employees:
        ...
        if emp.role != RoleEnum.EMPLOYEE:
            ...
            raise HTTPException(
                status_code=400, detail="Project can only be assigned to employees"
            )
...
def get_my_project(
    db: Session = Depends(get_db),
    current_user=Depends(require_roles([RoleEnum.EMPLOYEE])),
):
    ...
    for project in projects:
        ...
        if current_user["user_id"] in project.assigned_to:
            ...
            employees = db.query(User).filter(User.id.in_(project.assigned_to)).all()
...
def filter_by_department(
    ...
    employees = (
        db.query(User)
        .filter(User.department.in_(department_list), User.role == RoleEnum.EMPLOYEE)
        .all()
    )
...
def update_project_status(
    ...
    if current_user["user_id"] not in project.assigned_to:
        raise HTTPException(
            status_code=403, detail="You arenot assigned to this project"
        )
